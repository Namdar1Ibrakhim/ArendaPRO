...
public class MessagesController {
    ...
    @PostMapping("/send")
    public ResponseEntity sendMessage(@RequestBody MessageRequestDto messageRequstDto, @AuthenticationPrincipal User sender){
        messageService.sendMessage(messageRequstDto, sender);
        return new ResponseEntity(HttpStatus.OK);
    }

    @RequestMapping("/{user_id}")
    public ResponseEntity<List<Messages>> getMessagesByReceiver(@PathVariable Integer user_id, @AuthenticationPrincipal User user){
        return ResponseEntity.ok(messageService.getMessagesByReceiver(user_id, user));
    }
